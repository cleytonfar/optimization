...
persp(x, y, z, theta = 35, phi = 25, expand = 0.8, ticktype = "detailed", 
      xlab = "x", ylab="y", zlab = "f(x, y)", col = 'lightblue' , 
...
# Checking the results: 
#glm(formula = YY~XX, family = binomial)
...
# problem by multiplying the target-function by -1: 
...
# Results: 
list(minimum = (-1)*round(mL.prob01.BFGS$maximum, 5),
     estimate = round(mL.prob01.BFGS$estimate, 3))
...
# Results: 
list(minimum = (-1)*round(mL.prob01.Newton$maximum, 5),
     estimate = round(mL.prob01.Newton$estimate, 3))
...
# Results: 
list(Minimum = round(-1*mL.prob01.BHHH$maximum, 5),
           estimates = round(mL.prob01.BHHH$estimate, 3))
...
# Results:
list(log_likelihood_value = round(mL.LR.BFGS$maximum, 3),
     estimates = round(mL.LR.BFGS$estimate, 3))
...
# Results: 
list(log_likelihood = round(mL.LR.Newton$maximum, 3),
     estimates = round(mL.LR.Newton$estimate, 3))
...
# Results: 
list(Minimum = round(mL.LR.BHHH$maximum, 3),
     estimates = round(mL.LR.BHHH$estimate, 3))
...
# Results: 
list(log_likelihood = round(mL.logit.BFGS$maximum, 3),
     estiamtes = round(mL.logit.BFGS$estimate, 3))
...
# Results: 
list(log_likelihood = round(mL.logit.Newton$maximum, 3),
     estiamtes = round(mL.logit.Newton$estimate, 3))
...
# Results: 
list(log_likelihood = round(mL.logit.BHHH$maximum, 3),
     estiamtes = round(mL.logit.BHHH$estimate, 3))
